...
class TwitterCrawlerControllerTest < ActionController::TestCase
  ...
  test "ff_count" do
    ...
    tw2 = TwitterUser.find_by_screen_name(sn)
    assert_not_nil(tw2, "found ref user in db again")
    assert(tw2.friends.count() == friends_cnt, "friends.count() should be the same -> #{friends_cnt} == #{tw2.friends.count()}")
    assert(tw2.followers.count() == followers_cnt, "followers.count() should be the same -> #{followers_cnt} == #{tw2.followers.count()}")
  end  
